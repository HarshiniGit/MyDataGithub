...
# Store the movie name in variable named top_movie
  ...
  top_movie <-bollywood[1,which.max(bollywood$Tcollection)]
...
# Store the movie name in variable named top_2_movie
  ...
  top_2_movie <- bollywood[as.vector(order(-bollywood$Tcollection))[2],c(1)]
...
Bollywood_verdict <- summary(bollywood$Verdict) # To find the verdict factor vector using summary()
...
# Store the names of 4 movies in same sequence in movie_result vector
  ...
  movie_result <- bollywood[c(which.max(bollywood$Ocollection),
                              which.max(bollywood$Wcollection),
                              which.max(bollywood$Fwcollection),
                              which.max(bollywood$Tcollection)),1]
